...
fn get_data(subject: String) -> String {
  let quety = format!("PREFIX schema: <http://schema.org/>PREFIX imas: <https://sparql.crssnky.xyz/imasrdf/RDFs/detail/>SELECT * WHERE {{ imas:{} ?n ?o;}}order by (?n)", subject);
  let encoded_subject = form_urlencoded::Serializer::new(String::new())
